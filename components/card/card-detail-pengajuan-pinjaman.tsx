...
export default function CardDetailPengajuanPinjaman(props: ICardDetailPengajuanPinjaman) {
  return (
    <Card>
      <CardHeader className="flex flex-row items-center justify-between">
        <div className="flex items-center gap-2">
          <ArrowRightLeft className="text-indigo-600" />
          <CardTitle className="text-base font-semibold">Informasi Biaya Pinjaman</CardTitle>
        </div>
      </CardHeader>
      <CardContent className="grid gap-3 pt-2 text-sm text-muted-foreground">
        {props.data.pelunasan && props.data.pelunasan > 0 ? (
          <div className="flex justify-between">
            ...
            <span className="text-orange-500">{props.data.pinjamanId}</span>
          </div>
        ) : null}
        {props.data.pelunasan && props.data.pelunasan > 0 ? (
          <div className="flex justify-between">
        ...
        <div className="flex justify-between">
          ...
          <span className="text-red-600">- Rp {props.data.admin.toLocaleString("id-ID")}</span>
        </div>
        ...
        <div className="flex justify-between">
          ...
          <span>Rp {props.data.monthlyInstallment.toLocaleString("id-ID")}</span>
        </div>
      </CardContent>
    </Card>
  );
