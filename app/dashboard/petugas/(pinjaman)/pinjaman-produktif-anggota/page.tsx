import { columnPinjamanPetugas } from "@/components/columns/column-pinjaman";
import TableDateWrapper from "@/components/table/table-date-wrapper";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { RenderError } from "@/components/ui/render-error";
import { LABEL } from "@/lib/constan";
import { getPinjaman } from "@/lib/server/data/data-pinjaman";
...
export default async function PinjamanProduktifAnggota() {
  const [pinjaman] = await Promise.all([getPinjaman("PRODUKTIF")]);

  if (!pinjaman.ok || !pinjaman.data) {
    return RenderError("Pinjaman Produktif Anggota", LABEL.ERROR.DESCRIPTION);
  }
  ...
  return (
    <div className="flex flex-1 flex-col gap-4 p-4 md:gap-8 md:p-8">
      <div className="text-2xl font-medium">Pinjaman Produktif Anggota</div>
      ...
      {pendingPinjaman && (
        <TableDateWrapper
          header="Pengajuan Produktif Menunggu"
          ...
          columns={columnPinjamanPetugas}
        />
      )}
      {approvedPinjaman && (
        <TableDateWrapper
          header="Pinjaman Produktif Disetujui"
      ...
      {completedPinjaman && (
        <TableDateWrapper
          header="Pinjaman Produktif Selesai"
