...
export default async function SimpananQurban() {
  ...
  const [total, qurban] = await Promise.all([
    getSumSimpanan("QURBAN", noAnggota),
    getSimpananBerjangka("QURBAN", noAnggota),
  ]);

  if (!qurban.ok || !qurban.data || !total.ok) {
    return RenderError("Simpanan Qurban", LABEL.ERROR.DESCRIPTION);
  }

  return (
    <div className="flex flex-1 flex-col gap-4 p-4 md:gap-8 md:p-8">
      <div className="text-2xl font-medium">Simpanan Qurban</div>
